...
class CNN(nn.Module):
    def __init__(
        self, n_classes=10, n_filters=16, kernel_size=3, pool_size=2, optim=None
    ):
        super().__init__()
        ...
        self.model = nn.Sequential(
            nn.Conv2d(
                self.c, self.n_filters, self.kernel_size, padding=self.kernel_size // 2
            ),
            nn.ReLU(),
            nn.Conv2d(
                self.n_filters,
